...
// funkcja spania odpowiednio dosypiajaca w wypadku obslugi sygnalu
void milisleep(int ms)
{
    // pid_t pid = getpid();
    // srand(time(NULL)*pid);
    // int s = rand() % 91 + 10; // losowa liczba od 10 do 100
...
void child_work(char* data_to_process, int data_size, sigset_t oldmask, char* file_path, int number)
{
    ...
    {
        // funkcja zmniejszajaca/zwiekszjaca co druga litere
